...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if !usePrice {
		logContent = fmt.Sprintf("Model scale %.2f, completion scale %.2f, audio scale %.2f, audio completion scale %.2f, grouping scale %.2f",
			modelRatio, completionRatio.InexactFloat64(), audioRatio.InexactFloat64(), audioCompletionRatio.InexactFloat64(), groupRatio)
	} else {
		logContent = fmt.Sprintf("Model price %.2f, grouping multiplier %.2f", modelPrice, groupRatio)
	}
...
func PostClaudeConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) {
	...
	if totalTokens == 0 {
		...
		quota = 0
		logContent += fmt.Sprintf("(Possibly an upstream error)")
		logger.LogError(ctx, fmt.Sprintf("total tokens is 0, cannot consume quota, userId %d, channelId %d, "+
			"tokenId %d, model %s， pre-consumed quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, modelName, relayInfo.FinalPreConsumedQuota))
	} else {
		model.UpdateUserUsedQuotaAndRequestCount(relayInfo.UserId, quota)
	...
	if quotaDelta > 0 {
		logger.LogInfo(ctx, fmt.Sprintf("Post-deduction fee: %s (Actual consumption: %s, Pre-deduction fee: %s)",
			logger.FormatQuota(quotaDelta),
			logger.FormatQuota(quota),
			logger.FormatQuota(relayInfo.FinalPreConsumedQuota),
		))
	} else if quotaDelta < 0 {
		logger.LogInfo(ctx, fmt.Sprintf("Refund after prepayment deduction: %s (Actual consumption: %s, Prepayment: %s)",
			logger.FormatQuota(-quotaDelta),
			logger.FormatQuota(quota),
			logger.FormatQuota(relayInfo.FinalPreConsumedQuota),
		))
	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}, remaining balance: {{value}}, please recharge in time."
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else if notifyType == dto.NotifyTypeGotify {
				content = "{{value}}, the current remaining balance is {{value}}, please recharge in time."
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else {
				
				content = "{{value}}. The current remaining balance is {{value}}. To avoid affecting your usage, please recharge in a timely manner.<br/>Recharge link: <a href='{{value}}'>{{value}}</a>"
