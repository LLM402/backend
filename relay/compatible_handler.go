...
func postConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	if usage == nil {
		...
		extraContent += "(Possibly an error in the request)"
	}
	...
	if relayInfo.ResponsesUsageInfo != nil {
		if webSearchTool, exists := relayInfo.ResponsesUsageInfo.BuiltInTools[dto.BuildInToolWebSearchPreview]; exists && webSearchTool.CallCount > 0 {
			...
			extraContent += fmt.Sprintf("Web Search called %d times, context size %s, call cost %s",
		...
		dWebSearchQuota = decimal.NewFromFloat(webSearchPrice).
			Div(decimal.NewFromInt(1000)).Mul(dGroupRatio).Mul(dQuotaPerUnit)
		extraContent += fmt.Sprintf("Web Search called 1 time, context size %s, call cost %s",
	...
	if claudeWebSearchCallCount > 0 {
		...
		extraContent += fmt.Sprintf("Claude Web Search called %d times, costing %s",
	...
	if relayInfo.ResponsesUsageInfo != nil {
		if fileSearchTool, exists := relayInfo.ResponsesUsageInfo.BuiltInTools[dto.BuildInToolFileSearch]; exists && fileSearchTool.CallCount > 0 {
			...
			dFileSearchQuota = decimal.NewFromFloat(fileSearchPrice).
				...
				Div(decimal.NewFromInt(1000)).Mul(dGroupRatio).Mul(dQuotaPerUnit)
			extraContent += fmt.Sprintf("File Search called %d times, taking %s",
	...
	if ctx.GetBool("image_generation_call") {
		...
		extraContent += fmt.Sprintf("Image Generation Call costs %s", dImageGenerationCallQuota.String())
	}
	...
	if !relayInfo.PriceData.UsePrice {
		...
		if !dAudioTokens.IsZero() {
			...
			if audioInputPrice > 0 {
				...
				extraContent += fmt.Sprintf("Audio Input cost %s", audioInputQuota.String())
			}
		}
