	...
	if info.OriginTaskID != "" {
		...
		if originTask.ChannelId != info.ChannelId {
			...
			if channel.Status != common.ChannelStatusEnabled {
				return service.TaskErrorWrapperLocal(errors.New("The channel for this task has been disabled."), "task_channel_disable", http.StatusBadRequest)
			}
	...
	defer func() {
		...
		if info.ConsumeQuota && taskErr == nil {
			...
			if quota != 0 {
				...
				logContent := fmt.Sprintf("Operate %s", info.Action)
				
				if common.StringsContains(constant.TaskPricePatches, modelName) {
					logContent = fmt.Sprintf("%s, billed per use", logContent)
				} else {
					if len(info.PriceData.OtherRatios) > 0 {
						var contents []string
