...
func GetChannelKey(c *gin.Context) {
	...
	if err != nil {
		common.ApiError(c, fmt.Errorf("Failed to retrieve channel information: %v", err))
		return
	}
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if err := channel.ValidateSettings(); err != nil {
		return fmt.Errorf("Channel additional settings format error: %s", err.Error())
	}
	...
	if channel.Type == constant.ChannelTypeVertexAi {
		...
		if err != nil {
			return fmt.Errorf("The deployment region must be in standard JSON format, such as {\\\"default\\\": \\\"us-central1\\\", \\\"region2\\\": \\\"us-east1\\\"}")
		}

		if regionMap["default"] == nil {
			return fmt.Errorf("The deployment region must include the default field.")
		}
	}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if len(cleanKeys) == 0 {
		return nil, fmt.Errorf("The keys for batch adding Vertex AI cannot be empty.")
	}
				...
				if channel.Type == constant.ChannelTypeVertexAi && channel.GetOtherSettings().VertexKeyType != dto.VertexKeyTypeAPIKey {
					...
					if strings.HasPrefix(strings.TrimSpace(channel.Key), "[") {
						...
						if err != nil {
							c.JSON(http.StatusOK, gin.H{
								"success": false,
								"message": "Failed to add key parsing:" + err.Error(),
							})
							return
						}
						newKeys = array
...
func ManageMultiKeys(c *gin.Context) {
	...
	case "disable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No key index specified for disabling.",
			})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "The key has been disabled.",
		})
		return
	...
	case "enable_key":
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "The key has been enabled.",
		})
		return
	...
	case "delete_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "The key index to delete is not specified.",
			})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "The key has been deleted.",
		})
		return
	...
	case "delete_disabled_keys":
		...
		if deletedCount == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No automatic disable keys to delete.",
			})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": fmt.Sprintf("%d automatic disabled keys have been deleted.", deletedCount),
			"data":    deletedCount,
		})
		return
