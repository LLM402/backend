...
func UpdateSunoTaskAll(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("Channel #%d failed to update asynchronous task: %d", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTaskAll(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("Channel #%d has %d incomplete tasks: successfully retrieved %d tasks.", channelId, len(taskIds), string(responseBody)))
		return err
	}
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			...
			if err != nil {
				...
				if quota != 0 {
					...
					logContent := fmt.Sprintf("Asynchronous task execution failed %s, compensation %s", task.TaskID, logger.LogQuota(quota))
