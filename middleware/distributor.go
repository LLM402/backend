		...
		if ok {
			...
			if modelLimitEnable {
				...
				if !ok {
					...
					abortWithOpenAiMessage(c, http.StatusForbidden, "This token does not have access to any models.")
					return
				}
				...
				if _, ok := tokenModelLimit[matchName]; !ok {
					abortWithOpenAiMessage(c, http.StatusForbidden, "The token does not have access to the model."+modelRequest.Model)
					return
				}
			}
			...
			if shouldSelectChannel {
				...
				if strings.HasPrefix(c.Request.URL.Path, "/pg/chat/completions") {
					...
					if playgroundRequest.Group != "" {
						if !setting.GroupInUserUsableGroups(playgroundRequest.Group) && playgroundRequest.Group != userGroup {
							abortWithOpenAiMessage(c, http.StatusForbidden, "No access to this group.")
							return
						}
				...
				if err != nil {
					...
					message := fmt.Sprintf("Failed to get available channels for model %s under group %s (distributor): %s", showGroup, modelRequest.Model, err.Error())
				...
				if channel == nil {
					abortWithOpenAiMessage(c, http.StatusServiceUnavailable, fmt.Sprintf("No available channels (distributor) for model %s under group %s.", userGroup, modelRequest.Model), string(types.ErrorCodeModelNotFound))
					return
				}
			}
		}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		if relayMode == relayconstant.RelayModeMidjourneyTaskFetch ||
			...
			if midjourneyModel == "" {
				if !success {
					return nil, false, fmt.Errorf("Invalid request, unable to parse the model.")
				} else {
		...
		if c.Request.Method == http.MethodPost {
			...
			if strings.HasPrefix(contentType, "multipart/form-data") {
				...
				if err != nil {
					return nil, false, errors.New("Invalid video request," + err.Error())
				}
