...
func LogQuota(quota int) string {
	...
	case operation_setting.QuotaDisplayTypeCNY:
		...
		return fmt.Sprintf("¥%.6f limit", cny)
	case operation_setting.QuotaDisplayTypeCustom:
		...
		v := usd * rate
		return fmt.Sprintf("%s%.6f limit", symbol, v)
	case operation_setting.QuotaDisplayTypeTokens:
		return fmt.Sprintf("%d point limit", quota)
	default: // USD
		return fmt.Sprintf("$%.6f limit", q/common.QuotaPerUnit)
	}
}
